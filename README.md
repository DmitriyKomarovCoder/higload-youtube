...
## 3. Расчет нагрузки
### Нахождение ЦОДов
- Самый большой трафик идёт из Южной Азии, там будет находится датацентр в Индии, чтобы обеспечить основные страны, Индия, Пакистан.
- Также в России необходим датацентр, который будет работать, как на саму Россию, так и на страны СНГ, Европы.
- В США и Бразилии будет расположен 3 и 4 ый датацентр для обеспечения Северную и Южную Америку.

### DNS балансировка
- Будет использовать Latency-based DNS (AMAZON S53), в результате чего пользователю будет подбираться ближайший датацентр, будем исплоьзовать эту технологию для балансировки в рамках региона.
### BGP Anycast
- Внутри регионов будем ислопьзовать BGP Anycast (будем выдавать один ip для нескольких датацентров, отправлять пользователя к ближайшему датацентру)
- Будем использовать CDN сервера для отдачи статики (видео, информация о пользователях). Для этого будем кэшировать ролики в датацентрах и дальше рассылать по cdn серверам, также будем предоставлять провайдерам кэши для ускорения контента(ISP), чтобы снять нагрузку с cdn сервера.
  - Кеш сервер будет работать так, отдавать пользователю контент и в самое непиковое время по нагрузке загружать с cdn серверов новый контент.